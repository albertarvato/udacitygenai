...
def generate_app(get_processed_inputs, inpaint):
    ...
    def get_points(img, evt: gr.SelectData):
        global input_image
        # The first time this is called, preprocess and save the input image
    ...
    def run_sam():
        ...
        # Guard: ensure we have an image and at least one point before running SAM
        if input_image is None or len(input_points) == 0:
            gr.Warning("Please upload an image and click at least one point to segment with SAM.")
            # Return a placeholder annotated image so the UI updates gracefully without errors
            placeholder = (
                input_image.resize((IMG_SIZE, IMG_SIZE))
                if input_image is not None
                else Image.new("RGB", (IMG_SIZE, IMG_SIZE), "white")
            )
            empty_mask = np.zeros((IMG_SIZE, IMG_SIZE), dtype=bool)
            return (
                placeholder,
                [
                    (empty_mask, "background"),
                    (~empty_mask, "subject"),
                ],
            )
    ...
    def run(prompt, negative_prompt, cfg, seed, invert):
        ...
        # Guard: ensure user selected at least one point
        if input_image is None or len(input_points) == 0:
            gr.Warning("Select at least one point on the image before running inpaint.")
            return None
    ...
    def reset_points(*args):
        ...
        input_points.clear()
        # Also reset cached input image to require a fresh selection
        global input_image
        input_image = None
